...
- 当前已有近期工艺路线 migration，但尚未形成覆盖历史 schema 的完整迁移基线；本机开发环境仍使用 `prisma db push`。
- 正式环境不能直接沿用本机 `db push` 策略。应先对线上 schema 建立并审核 baseline migration，再改为 `prisma migrate deploy`。
