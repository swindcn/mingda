...
- 模具档案表单底部显示“是否有芯盒”。
- 勾选后展开芯盒资料，随模具档案一起提交。
- 芯盒字段：编码、名称、关联模具、穴数、使用寿命、已用次数、芯盒图片、备注。
- 芯盒“穴数”是芯盒工装一次可形成的型芯数量，必须为大于 `0` 的整数，新增默认 `1`；它独立于模具型腔数，也不能替代 BOM 芯件比。
...
- `CoreBoxMaster` 通过 `moldCode` 关联 `MoldMaster`，保存芯盒图片、穴数、寿命、已用次数。
...
- “保质期（小时）”也是 BOM 版本与芯盒的关系属性，保存在 `CastingBomVersionCoreBox.shelfLifeHours`；允许为空，填写时必须大于 `0`，用于后续制芯任务计算砂芯失效时间和齐套校验。
- 芯盒穴数、BOM 芯件比、BOM 保质期是三个独立字段：穴数描述工装能力，芯件比描述产品需求，保质期描述该产品工艺下砂芯可保存时长。
