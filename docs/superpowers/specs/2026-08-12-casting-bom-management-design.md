...
- 模具选项仅包含 `itemCode` 等于 BOM 产品编码且状态为启用的模具。芯盒选项仅包含已选模具下状态为启用的芯盒。
- BOM 草稿保存时后端重新验证产品、模具、芯盒关系，不能只依赖前端过滤。
...
1. 产品基本信息与重量参数。
2. 生产工装：生产模具多选、芯盒工装多选，芯盒按已选模具联动过滤。
...
13. 产品选择后只显示该产品启用的模具；选择模具后只显示其启用芯盒。
14. 提交不匹配产品的模具，或不属于已选模具的芯盒时，后端拒绝。
