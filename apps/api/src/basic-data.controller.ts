...
const defaultDictionaries = {
  ...
  equipmentTypes: ['熔炼炉', '浇注包', '球化包', '烘干设备', '落砂', '清理', '抛丸', '打磨', '切割', '其他设备'],
  chemicalElements: [
    { name: 'C', unit: '%' },
    { name: 'Si', unit: '%' },
    { name: 'Mn', unit: '%' },
...
export class BasicDataController {
  ...
  async updateDictionaries(@Body() body: DictionaryBody) {
    const next = {
      ...
      equipmentTypes: Array.isArray(body.equipmentTypes) ? body.equipmentTypes : defaultDictionaries.equipmentTypes,
  ...
  private async getDictionaries() {
    ...
    return {
      ...
      equipmentTypes: map.has('equipmentTypes') ? stringArray(map.get('equipmentTypes')) : defaultDictionaries.equipmentTypes,
