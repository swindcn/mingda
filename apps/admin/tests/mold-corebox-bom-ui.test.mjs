...
test('mold archive uses the multi-corebox editor', () => {
  ...
  assert.match(editor, /穴数/)
  assert.match(editor, /cavityCount/)
  assert.match(editor, /cavityCount: 1/)
  assert.match(page, /mold\.corebox\.create/)
  assert.match(page, /mold\.corebox\.edit/)
  assert.match(editor, /Form\.useWatch\('coreBoxes', form\)/)
  assert.match(editor, /canCreate/)
  assert.match(editor, /canEdit/)
  assert.match(page, /if \(!canCreate\)/)
