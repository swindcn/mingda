...
后续开发人员请先阅读 [项目开发交接说明](docs/product/project-handoff.md)。该文档记录当前 Git 状态、环境配置、模块边界、已知风险和建议接手顺序。
...
- 设备配置：维护熔炼炉、射芯机、造型线、浇注和清理设备；设备适用范围由配方和工艺路线关系表达。
...
- 材质牌号：维护材料类别/类型、版本、化学成分、力学性能、工艺要求和标准版本，明细为真实独立数据结构。
- 熔炼配方：关联材质牌号、适用熔炼设备和原材料，维护目标成分快照、配比、版本、克隆和状态。
- 铸造 BOM：为成品/半成品维护材质、重量与收得率、物理用料、生产模具和芯盒关系，支持版本、克隆和生产需求计算。
- 工序管理：维护统一工序库、工段、报工模式、质量控制点和浇注汇合标志。
- 工艺路线：一条路线可绑定多个成品/半成品，通过自由画布和有向连线表达并行、汇合和前后依赖，每个节点可多选设备。
- 缺陷代码库：维护缺陷编码、名称、分类和易发工序。

### 生产管理与熔炼执行

- 生产工单：成品/半成品提交后锁定已生效 BOM、默认工艺路线、材质和重量快照，直接进入待排池。
- 合炉排产：按材质隔离，支持一单多炉、一炉多单、按整数件拆分，校验熔炼车间、设备容量、配方和执行班组。
- 设备排程：配方时长换算设备占用区间，展示设备卡片、容量占比和 24 小时时间线；待生产炉次可按 15 分钟吸附调整。
- 熔炼执行：选择实际熔炉开始生产，可多次转运并绑定浇注包/球化包，完成时按转运累计预填最终重量。
